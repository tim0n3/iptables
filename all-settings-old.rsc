...
(202 messages not shown)
feb/20/2022 17:07:08 system,error,critical router was rebooted without proper shutdown
feb/20/2022 17:07:07 system,error,critical router was rebooted without proper shutdown
feb/20/2022 17:07:07 system,error,critical router was rebooted without proper shutdown
feb/20/2022 17:07:06 system,error,critical router was rebooted without proper shutdown
feb/20/2022 17:07:07 system,error,critical router was rebooted without proper shutdown
feb/20/2022 17:07:07 system,error,critical router was rebooted without proper shutdown
feb/20/2022 17:07:07 system,error,critical router was rebooted without proper shutdown
feb/20/2022 17:07:07 system,error,critical router was rebooted without proper shutdown
[admin@AMS003-TANDEM-NORTH-FUME-EXTRACTION-FAN-LTE] > export
# mar/24/2022 14:37:48 by RouterOS 6.45.9
# software id = YZZG-E2L7
#
# model = RBLHGR
# serial number = D6B80D48CC67
/interface bridge
add admin-mac=48:8F:5A:76:1A:C4 auto-mac=no comment=defconf name=bridge
/interface ethernet
set [ find default-name=ether1 ] mac-address=48:8F:5A:76:1A:C4
...
/interface lte
set [ find ] allow-roaming=yes apn-profiles=axxess mac-address=AC:50:43:1A:EE:FD mtu=1480 name=lte1
...
add name="10. no-mark_" packet-mark=no-mark parent=UP queue=default
add name="10. no-mark" packet-mark=no-mark parent=DOWN queue=redCustom
...
add address=156.38.145.16/28 list=support_external
...
add address=1.0.0.1 comment="AXXESS upstream DNS servers" list=public_DNS
...
add action=drop chain=forward comment="This rule blocks access to facebook" dst-address-list=facebook log=yes log-prefix="[Block] facebook"
...
add action=accept chain=forward comment="### DEBUG forwarding DNS not required - Address-list of Permitted Public DNS resolvers " disabled=yes in-interface=lte1 src-address-list=public_DNS
...
add chain=input comment="Accept all connections from local network" in-interface=bridge
...
add action=masquerade chain=srcnat comment="src-NAT for Allowed clients" log-prefix=egress_lan_NAT out-interface-list=WAN
...
add action=dst-nat chain=dstnat comment="DNAT to RPi SSH" disabled=yes dst-port=22 log=yes log-prefix=SSH_to_RPi4 protocol=tcp src-address-list=support_external to-addresses=192.168.8.200 to-ports=22
add action=accept chain=dstnat comment="dst-nat for allowed Public_IP's" src-address-list=support_external
/ip firewall raw
...
set ftp disabled=yes
set www address=105.23.225.106/32,37.48.118.94/32,34.90.83.14/32 disabled=yes
...
set time-zone=+02:00
/system identity
set name=AMS003-TANDEM-NORTH-FUME-EXTRACTION-FAN-LTE
/system leds
...
add down-script="log info \"Netwatch missed a ping to 8.8.8.8 - starting 5 minute timeout script\" ; /system script run watchdog\r\
    \n" host=8.8.8.8
[admin@AMS003-TANDEM-NORTH-FUME-EXTRACTION-FAN-LTE] > 
